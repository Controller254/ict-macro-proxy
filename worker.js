...
// ForexFactory / FairEconomy limits weekly calendar exports to ~2 requests
// per 5 minutes PER IP, shared across .json/.xml/.csv/.ics. Exceeding it
// returns an HTML "Request Denied" page instead of JSON. We cache the
// response in-memory (per Worker isolate) for 5 minutes to stay well under
// that limit regardless of how many clients hit our /calendar endpoint.
let calendarCache = { data: null, fetchedAt: 0 };
const CALENDAR_CACHE_MS = 5 * 60 * 1000;

// Myfxbook's Community Outlook (retail positioning) requires an authenticated
// session — there is no public, key-free JSON endpoint for this data (FXSSI
// has no API at all; its page is rendered client-side via JS, so server-side
// scraping returns nothing). To enable /sentiment, set MYFXBOOK_EMAIL and
// MYFXBOOK_PASSWORD as Worker secrets (free Myfxbook account, no card):
//   npx wrangler secret put MYFXBOOK_EMAIL
//   npx wrangler secret put MYFXBOOK_PASSWORD
// Without those secrets, /sentiment returns a clear "not configured" error
// instead of silently failing.
let myfxbookSession = { id: null, fetchedAt: 0 };
const MYFXBOOK_SESSION_TTL_MS = 50 * 60 * 1000; // sessions last ~1hr, refresh at 50min
